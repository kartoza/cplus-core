...
from ..models.base import Scenario, Activity, SpatialExtent, LayerType, NcsPathway
...
class TaskConfig(object):
    ...
    def get_activity(self, activity_uuid: str) -> typing.Union[Activity, None]:
        ...
        activity = None
        filtered = [
            act for act in self.analysis_activities if str(act.uuid) == activity_uuid
        ]
        if filtered:
            activity = filtered[0]
        return activity
    ...
    def from_dict(cls, data: dict) -> typing.Self:
        ...
        :param data: Settings dictionary
        :type data: dict
        ...
        config.snap_rescale = data.get("snap_rescale", DEFAULT_VALUES.snap_rescale)
        config.snap_method = data.get("snap_method", DEFAULT_VALUES.snap_method)
        config.sieve_enabled = data.get("sieve_enabled", DEFAULT_VALUES.sieve_enabled)
        ...
        for priority_layer in config.priority_layers:
            ...
            if layer_uuid in config.priority_uuid_layers:
                config.priority_uuid_layers[layer_uuid].append(priority_layer_uuid)
            else:
                config.priority_uuid_layers[layer_uuid] = [priority_layer_uuid]
        ...
        for activity in _activities:
            ...
            for m_priority_layer in m_priority_layers:
                ...
                if not m_priority_uuid:
                    continue
                m_priority_layer_uuid = m_priority_layer.get("layer_uuid", None)
            ...
            for pathway in pathways:
                ...
                pw_uuid = uuid.UUID(pw_uuid_str) if pw_uuid_str else uuid.uuid4()
                ...
                if pw_layer_uuid:
                    if pw_layer_uuid in config.pathway_uuid_layers:
                        config.pathway_uuid_layers[pw_layer_uuid].append(str(pw_uuid))
                    else:
                        config.pathway_uuid_layers[pw_layer_uuid] = [str(pw_uuid)]
                carbon_uuids = pathway.get("carbon_uuids", [])
                for carbon_uuid in carbon_uuids:
                    if carbon_uuid in config.carbon_uuid_layers:
                        config.carbon_uuid_layers[carbon_uuid].append(str(pw_uuid))
                    else:
                        config.carbon_uuid_layers[carbon_uuid] = [str(pw_uuid)]
        ...
        config.base_dir = data.get("base_dir", "")
        return config
